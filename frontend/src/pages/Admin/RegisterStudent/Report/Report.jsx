...
const Report = (props) => {
    ...
    const arr = selectedMedicine.map((item, index) => {
      if (index === ind) {
        if (parseInt(item.quantity) < parseInt(event.target.value)) {
          toast.error("You have not that much meddicine in the store")
          return { ...item }
        }
  ...
  const handleOnSubmit = async () => {
    ...
    if (checkInputInValid()) return toast.error("Please enter all the fields.")
     await axios.post(`http://localhost:4000/api/history/add`,{roll:props.studentDetail.roll,student:props.studentDetail._id,medicines:selectedMedicine},{withCredentials:true}).then((resp)=>{
